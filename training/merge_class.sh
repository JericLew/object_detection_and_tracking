...
do
    ...
    if [ -f "$file_path" ]; then
        ...
        python ~/tracking_ws/training/frame_extractor/frame_extractor.py "$file_path" "$destination_dir"
    fi
done
...
do
    ...
    if [ -f "$file_path" ]; then
        ...
        # Run the Python command with the file path
        python ~/tracking_ws/training/matlab_convertor/matlab_to_yolo_labels.py "$file_path" "$destination_dir"
    fi
done
...
# FOR label merge
python ~/tracking_ws/training/merge_class/merge_class.py ~/datasets/merge_class/labels/

# FOR train / val split
python ~/tracking_ws/training/train_val_splitter/train_val_splitter_random.py ~/datasets/merge_class 0.7