...
def main(args=None):
    object_detector = objectTracker()
    while 1:
        ret, current_frame = object_detector.cap.read()

        result_boxes, result_confidences, result_class_ids = object_detector.detect(current_frame)
        object_detector.draw_bbox(current_frame, result_boxes, result_class_ids, tracking=False)
